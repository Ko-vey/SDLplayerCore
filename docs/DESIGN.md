...
本文档旨在深入阐述 `SDLplayerCore` 的内部架构、线程模型和核心功能的实现细节。主要目的在于：记录并提供项目当前状态的架构参考、为新加入开发者提供一份快速上手指南，并为未来可能的代码重构、功能扩展或二次开发提供可靠的设计依据和参考。

**目标读者**

本文档主要面向具备 C++11 多线程编程基础，并对 FFmpeg 和 SDL2 有初步了解的开发者。
...
- **经典多线程架构**：采用经典的“生产者-消费者”模型，将复杂播放流程解耦为一组职责单一的线程（解封装、音/视频解码、音/视频渲染），通过线程安全的队列进行通信。

- **音频主时钟策略**：以音频播放进度作为整个播放器的同步基准。视频流和外部事件全部向音频时钟对齐，以实现流畅的音画同步。
...
| 设计要点  | 核心价值与说明     |
| :------------ | :---------------- |
| **音频主时钟**   | **以音频播放进度为同步基准。** 此策略能显著提升用户体验。当媒体文件缺少音频轨时，系统会自动退化为外部时钟。          |
...
> **本节要点**
>
> - **核心策略**：采用“**音频时钟为主，外部时钟为辅**”的同步策略，所有视频帧的渲染时机都以 音频的播放进度/外部系统时钟 为基准。
> - **时钟计算**：`ClockManager` 模块负责管理时钟。音频时钟 = `最新音频帧PTS - SDL缓冲区内未播放时长`。无音频时，则自动退化为外部系统时钟。
...
2. **视频 → 时钟**：视频线程**只读**时钟，不修改主时钟，以防时钟失真。
...
- **音频主导同步**：以音频时钟为基准，确保了体验流畅、自然的音画同步。
...
- **扩展格式支持**：
  ...
  - 加载并渲染字幕文件 (如 `.srt`, `.ass`)。
  - 支持网络流媒体播放 (如 `HLS`, `RTSP`)。
- **性能优化**：

  - 引入硬件加速解码，以降低高分辨率视频播放时的 CPU 占用。
- **算法探索**：

  - 研究更高级的动态同步算法，以应对复杂网络环境。
