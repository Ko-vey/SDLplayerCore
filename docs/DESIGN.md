...
>  图片文件 (`.svg`) 用于在文档中直接展示。对应的源文件 (`.drawio` 以及 部分以`.md` 格式储存的 `mermaid` 源代码) 存放在 `docs/assets/` 目录下。
> 如需修改图表，请优先编辑 `mermaid` 源代码、导入`.drawio` 源文件细调后导出为 `.svg` 图片，然后将同名的各个新文件一并提交。
...
在本项目中，设计并实现了 `PacketQueue` 和 `FrameQueue`（两者设计思想和结构类似，本章节以 `PacketQueue` 为例），它们是基于生产者-消费者模式的高度并发组件。其核心职责是：
...
![缓存队列对消费者的阻塞示意图](assets/pic5-queue_thread-consumer_block.svg)
